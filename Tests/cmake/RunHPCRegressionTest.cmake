# CMake script to run a PIAFS regression test on HPC systems (CPU-only)
# This is used on Dane and similar CPU-only HPC systems with job schedulers
#
# Required variables:
...
# - MPI_RANKS: Number of MPI ranks to use (1, 2, or 4)
...
message(STATUS "Setting up HPC test ${TEST_NAME} with ${MPI_RANKS} MPI rank(s)...")
...
# Create a custom run script that uses srun for HPC scheduling
set(RUN_SCRIPT "${TEST_DIR}/run_hpc.sh")
file(WRITE "${RUN_SCRIPT}" "#!/bin/bash\n\n")
file(APPEND "${RUN_SCRIPT}" "rm -rf *core* op* *.dat plots out.log\n")
file(APPEND "${RUN_SCRIPT}" "export PIAFS_USE_GPU=0\n\n")
...
file(APPEND "${RUN_SCRIPT}" "echo \"Running PIAFS with ${MPI_RANKS} MPI rank(s)...\"\n")
file(APPEND "${RUN_SCRIPT}" "srun -p pdebug -n ${MPI_RANKS} ${PIAFS_EXEC} 2>&1 | tee -a out.log\n")
...
# Run the test
message(STATUS "Running HPC test ${TEST_NAME} with ${MPI_RANKS} MPI rank(s)...")
execute_process(
  COMMAND ${RUN_SCRIPT}
...
# Report results
message(STATUS "Test ${TEST_NAME} complete (${MPI_RANKS} MPI rank(s)): ${PASS_COUNT}/${COMPARISON_COUNT} comparisons passed")
