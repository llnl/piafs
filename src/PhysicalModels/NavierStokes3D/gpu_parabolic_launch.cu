...
)
{
  ...
  GPU_KERNEL_LAUNCH(gpu_ns3d_viscous_flux_x_kernel, gridSize, blockSize)(
    FViscous, Q, QDerivX, QDerivY, QDerivZ, nvars, npoints, Tref, T0, TS, TA, TB, inv_Re, inv_gamma_m1, inv_Pr
  );
...
void gpu_launch_scale_array_with_dxinv(
  double *QDeriv, const double *dxinv_array, int nvars, int npoints,
  int ndims, const int *dim, const int *stride_with_ghosts, int ghosts,
  int dir, int dir_offset, int blockSize
)
{
#ifdef GPU_NONE
  /* CPU fallback */
  for (int idx = 0; idx < npoints; idx++) {
    ...
    for (int d = 0; d < ndims; d++) {
      p += current_idx[d] * stride_with_ghosts[d];
    }
  ...
  GPU_KERNEL_LAUNCH(gpu_scale_array_with_dxinv_kernel, gridSize, blockSize)(
    QDeriv, dxinv_array, nvars, npoints, ndims, dim, stride_with_ghosts, ghosts, dir, dir_offset
  );
  GPU_CHECK_ERROR(GPU_GET_LAST_ERROR());
#endif
}
...
void gpu_launch_add_scaled_derivative(
  double *par, const double *FDeriv, const double *dxinv_array, int nvars,
  int npoints_interior, int ndims, const int *dim,
  const int *stride_with_ghosts, int ghosts, int dir, int dir_offset, int blockSize
)
{
#ifdef GPU_NONE
  /* CPU fallback */
  for (int idx = 0; idx < npoints_interior; idx++) {
    int current_idx[3];
    int temp_idx = idx;
    for (int d_rev = ndims - 1; d_rev >= 0; --d_rev) {
...
#else
  if (blockSize <= 0) blockSize = DEFAULT_BLOCK_SIZE;
  int gridSize = (npoints_interior + blockSize - 1) / blockSize;
  GPU_KERNEL_LAUNCH(gpu_add_scaled_derivative_kernel, gridSize, blockSize)(
    par, FDeriv, dxinv_array, nvars, npoints_interior, ndims, dim, stride_with_ghosts, ghosts, dir, dir_offset
  );
  GPU_CHECK_ERROR(GPU_GET_LAST_ERROR());
#endif
}
