...
int NavierStokes3DWriteChem (void*,void*,double);
int NavierStokes3DPreStep (double*,void*,void*,double,double);
                              ...
                              void *m  /*!< MPI object of type #MPIVariables */ )
{
  ...
  physics->nvars = _NS3D_NVARS_;

  if (physics->include_chem) {
    if (!strcmp(physics->write_op,"yes")) solver->PhysicsOutput = NavierStokes3DWriteChem;
    ...
    if (physics->Re > 0) {
      if (!mpi->rank) {
        printf("NavierStokes3DInitialize(): Computing Reynolds and Prandtl's number based on photochemistry setup; ignoring values in physics.inp.\n");
      }
  ...
  if (solver->nvars != physics->nvars) {
    fprintf(stderr,"Error in NavierStokes3DInitialize(): nvars has to be %d in solver.inp.\n",physics->nvars);
    return(1);
  }

  if (!mpi->rank) {
    printf("NavierStokes3D parameters:\n");
    printf("    gamma: %1.4e\n", physics->gamma);
