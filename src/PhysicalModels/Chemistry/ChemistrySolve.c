                     ...
                     double  a_t   /*!< Current simulation time */ )
{
  ...
  for (iz = 0; iz < nz; iz++) {
    ...
    while (!done) {
      ...
      uchem[5] = params->nv_CO2[nz*p+iz];
      uchem[6] = params->nv_hnu[nz*p+iz];
      ...
      if (!strcmp(params->ti_scheme, "RK4")) {
        ...
        params->nv_CO2[nz*p+iz] += a_dt*(f1[4]+2.0*f2[4]+2.0*f3[4]+f4[4])/6.0;
        ...
        params->nv_CO2[nz*p+iz] += a_dt * fchem[4];
