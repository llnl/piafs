...
for s in range(nsims):
  ...
  plot = ax.pcolor(x2d, y2d, rho[:,j0,:], cmap=colormap)
  ax.set_title('rho at j={:}, y={:.3}'.format(j0,y[j0]))
  fig.colorbar(plot, ax=ax)
  if nsims > 1:
    plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_rho_xt_'+f'j{j0:02d}'+'.png'
  else:
    plt_fname = plt_dir_name+'/fig_rho_xt_'+f'j{j0:02d}'+'.png'
  ...
  ax = plt.axes()
  ax.set( xlim=(np.min(x), np.max(x)), ylim=(0, t_final) )
  plot = ax.pcolor(x2d, y2d, pressure[:,j0,:], cmap=colormap)
  ax.set_title('pressure at j={:}, y={:.3}'.format(j0,y[j0]))
  fig.colorbar(plot, ax=ax)
  if nsims > 1:
    plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_pressure_xt_'+f'j{j0:02d}'+'.png'
  else:
    plt_fname = plt_dir_name+'/fig_pressure_xt_'+f'j{j0:02d}'+'.png'
  print('Saving %s' % plt_fname)
  plt.savefig(plt_fname)
  plt.close()
  ...
  ax = plt.axes()
  ax.set( xlim=(np.min(x), np.max(x)), ylim=(0, t_final) )
  plot = ax.pcolor(x2d, y2d, temperature[:,j0,:], cmap=colormap)
  ax.set_title('temperature at j={:}, y={:.3}'.format(j0,y[j0]))
  fig.colorbar(plot, ax=ax)
  if nsims > 1:
    plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_temperature_xt_'+f'j{j0:02d}'+'.png'
  else:
    plt_fname = plt_dir_name+'/fig_temperature_xt_'+f'j{j0:02d}'+'.png'
  print('Saving %s' % plt_fname)
  plt.savefig(plt_fname)
  plt.close()
