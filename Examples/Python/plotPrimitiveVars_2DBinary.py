...
if solver_inp_data['op_overwrite'] == 'no':
  ...
  for i in range(n_snapshots):
    for s in range(nsims):
      ...
      cons_sol = np.transpose(solution_snapshots_sim.reshape(n_snapshots,size[1],size[0],nvars))
      rho = cons_sol[0,:,:,:]
      ...
      plot = ax.pcolor(x2d, y2d, rho[:,:,i], cmap=colormap)
      ax.set_title('rho, t={:.3}'.format(i*dt_snapshots))
      fig.colorbar(plot, ax=ax)
      if nsims > 1:
        plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_rho_'+f'{i:05d}'+'.png'
      else:
        plt_fname = plt_dir_name+'/fig_rho_'+f'{i:05d}'+'.png'
      ...
      ax = plt.axes()
      ax.set( xlim=(np.min(x), np.max(x)), ylim=(np.min(y), np.max(y)) )
      plot = ax.pcolor(x2d, y2d, uvel[:,:,i], cmap=colormap)
      ax.set_title('uvel, t={:.3}'.format(i*dt_snapshots))
      fig.colorbar(plot, ax=ax)
      if nsims > 1:
        plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_uvel_'+f'{i:05d}'+'.png'
      else:
      ...
      ax = plt.axes()
      ax.set( xlim=(np.min(x), np.max(x)), ylim=(np.min(y), np.max(y)) )
      plot = ax.pcolor(x2d, y2d, vvel[:,:,i], cmap=colormap)
      ax.set_title('vvel, t={:.3}'.format(i*dt_snapshots))
      fig.colorbar(plot, ax=ax)
      if nsims > 1:
        plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_vvel_'+f'{i:05d}'+'.png'
      else:
      ...
      ax = plt.axes()
      ax.set( xlim=(np.min(x), np.max(x)), ylim=(np.min(y), np.max(y)) )
      plot = ax.pcolor(x2d, y2d, pressure[:,:,i], cmap=colormap)
      ax.set_title('pressure, t={:.3}'.format(i*dt_snapshots))
      fig.colorbar(plot, ax=ax)
      if nsims > 1:
        plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_pressure_'+f'{i:05d}'+'.png'
      else:
  ...
  for s in range(nsims):
    solution_snapshots_sim = solution_snapshots[s*n_snapshots:(s+1)*n_snapshots]
    cons_sol = np.transpose(solution_snapshots_sim.reshape(size[1],size[0],nvars))
    ...
    ax = plt.axes()
    ax.set( xlim=(np.min(x), np.max(x)), ylim=(np.min(y), np.max(y)) )
    plot = ax.pcolor(x2d, y2d, uvel[:,:], cmap=colormap)
    ax.set_title('uvel, t={:.3}'.format(t_final))
    fig.colorbar(plot, ax=ax)
    if nsims > 1:
      plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_uvel.png'
    else:
    ...
    ax = plt.axes()
    ax.set( xlim=(np.min(x), np.max(x)), ylim=(np.min(y), np.max(y)) )
    plot = ax.pcolor(x2d, y2d, vvel[:,:], cmap=colormap)
    ax.set_title('vvel, t={:.3}'.format(t_final))
    fig.colorbar(plot, ax=ax)
    if nsims > 1:
      plt_fname = plt_dir_name+'/fig_'+f'{s:02d}'+'_vvel.png'
    else:
