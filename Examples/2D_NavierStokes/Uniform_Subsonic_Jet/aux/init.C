...
int main()
{
  ...
  double Ptot = 1.0e7; // [Pa]; total gas pressure
  double ntot = 1.0e26; // [m^{-3}; total number density
  double Ti = Ptot / (kB * ntot); // [K]; initial temperature
  ...
  if ((f_CO2 > 1.0) || (f_CO2 < 0.0)) {
    fprintf(stderr,"ERROR in chemistry.inp: f_CO2 is not between 0.0 and 1.0 !!!\n");
    return 1;
  }
  if ((f_O3 > 1.0) || (f_O3 < 0.0)) {
    fprintf(stderr,"ERROR in chemistry.inp: f_O3 is not between 0.0 and 1.0 !!!\n");
    return 1;
  }
  if (Ptot != inp_Ptot) {
