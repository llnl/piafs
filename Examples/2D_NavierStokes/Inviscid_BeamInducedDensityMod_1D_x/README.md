# Inviscid Beam-Induced Density Modulation (1D in x)

## Overview

Simulation of density modulation induced by photo-chemical heating from a laser beam in inviscid flow. This test case demonstrates the coupling between inviscid fluid dynamics and chemical heating source terms. This is a 1D problem (variation in x only) solved using the 2D Navier-Stokes solver with y as a dummy dimension and viscous terms set to zero.

## Governing Equations

2D Navier-Stokes equations (inviscid) with chemical heating source term:
...
1. **Generate the initial solution:**
   ```bash
   cd Examples/2D_NavierStokes/Inviscid_BeamInducedDensityMod_1D_x
   g++ aux/init.C -o INIT
   ./INIT
   ```
...
- Although the 2D Navier-Stokes solver is used, this is effectively a 1D inviscid problem with variation only in the x-direction
- This is the inviscid version of the beam-induced density modulation problem, useful for isolating the effects of chemical heating without viscous dissipation
