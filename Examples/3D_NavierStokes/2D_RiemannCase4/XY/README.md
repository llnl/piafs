# 2D Riemann Problem (Case 4) - XY Plane

## Overview

Simulation of the 2D Riemann problem (Configuration 4) along the XY plane using the 3D Navier-Stokes solver. This is a classic test case for compressible flow solvers featuring complex wave interactions including shock waves, contact discontinuities, and rarefaction waves. The problem is solved as a 2D problem in the XY plane, with Z as a dummy dimension.
...
- Z is a dummy dimension since this is effectively a 2D problem in the XY plane

## Initial Conditions

The 2D Riemann problem consists of four constant states in four quadrants separated by discontinuities at x=0 and y=0. See the reference paper for specific initial values for Case 4.
...
| size | 201 201 21 | Grid size (for 1D) or grid dimensions separated by spaces (for 2D/3D) |
| iproc | 5 5 1 | Number of MPI ranks in each dimension (for parallel runs) |
...
```
6
extrapolate      0     1     0      0  -0.5    0.5  -0.5    0.5
extrapolate      0    -1     0      0  -0.5    0.5  -0.5    0.5
extrapolate      1     1  -0.5    0.5     0      0  -0.5    0.5
extrapolate      1    -1  -0.5    0.5     0      0  -0.5    0.5
periodic         2     1  -0.5    0.5  -0.5    0.5     0      0
periodic         2    -1  -0.5    0.5  -0.5    0.5     0      0
```
...
1. **Generate the initial solution:**
   ```bash
   cd Examples/3D_NavierStokes/2D_RiemannCase4/XY
   g++ aux/init.C -o INIT
   ./INIT
   ```
