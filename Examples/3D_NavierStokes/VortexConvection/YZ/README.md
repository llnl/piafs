# Inviscid Isentropic Vortex Convection - YZ Plane

## Overview

Simulation of inviscid isentropic vortex convection where the vortex axis is along the X-axis and convects along the Y-direction. This problem is equivalent to a 2D problem in the YZ plane. This is a classic test case for validating the accuracy and low-dissipation properties of numerical schemes. This test case is inviscid and therefore does not test the viscous terms implementation.
...
Vortex centered at `y0 = 5.0`, `z0 = 5.0`:
...
where `r^2 = (y-y0)^2 + (z-z0)^2` and `b` is the vortex strength parameter.
...
| size | 60 60 60 | Grid size (for 1D) or grid dimensions separated by spaces (for 2D/3D) |
| iproc | 3 3 3 | Number of MPI ranks in each dimension (for parallel runs) |
...
| file_op_iter | 40 | Iterations between file output |
...
```
6
periodic      0     1     0     0      0   10.0     0     10.0
periodic      0    -1     0     0      0   10.0     0     10.0
periodic      1     1     0  10.0      0      0     0     10.0
periodic      1    -1     0  10.0      0      0     0     10.0
periodic      2     1     0  10.0      0   10.0     0        0
periodic      2    -1     0  10.0      0   10.0     0        0
```
...
1. **Generate the initial and exact solutions:**
   ```bash
   cd Examples/3D_NavierStokes/VortexConvection/YZ
   g++ aux/init.C -o INIT
   ./INIT
