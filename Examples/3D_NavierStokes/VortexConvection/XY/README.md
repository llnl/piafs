# Inviscid Isentropic Vortex Convection - XY Plane

## Overview

Simulation of inviscid isentropic vortex convection where the vortex axis is along the Z-axis and convects along the X-direction. This problem is equivalent to a 2D problem in the XY plane. This is a classic test case for validating the accuracy and low-dissipation properties of numerical schemes. This test case is inviscid and therefore does not test the viscous terms implementation.
...
where `r^2 = (x-x0)^2 + (y-y0)^2` and `b` is the vortex strength parameter.
...
1. **Generate the initial and exact solutions:**
   ```bash
   cd Examples/3D_NavierStokes/VortexConvection/XY
   g++ aux/init.C -o INIT
   ./INIT
